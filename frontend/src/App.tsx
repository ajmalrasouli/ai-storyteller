...
export default function App() {
  ...
  const audioRef = useRef<HTMLAudioElement | null>(null);
  const abortControllerRef = useRef<AbortController | null>(null);
  // Title is now auto-generated by the backend
  ...
  const handleGenerateStory = async () => {
    ...
    try {
      ...
      if (charactersArray.length === 0) {
        toast.error("Please add at least one character to your story!");
        setLoading(false);
        return;
      }
      ...
      toast.error("Sorry, there was an error creating your story. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleToggleFavorite = async (storyId: number) => {
    try {
      const { isFavorite } = await toggleFavorite(storyId);
      // Update local state
        ...
        {activeTab === 'create' && (
          <div className="adventure-layout">
            ...
            <div className="create-story-form">
              {/* Title field removed - now auto-generated by the backend */}
              <CreateStorySection
                theme={theme}
